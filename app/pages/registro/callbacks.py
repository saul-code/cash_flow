...
def r_guardar(
    n_clicks, fecha, monto, desc, tarjeta_id, cat_id, tipo, es_msi, meses, refresh
):
    ...
    try:
        ...
        # 1. 🔍 Obtenemos la tarjeta para saber su día de corte
        tarjeta = session.query(Tarjeta).get(int(tarjeta_id))
        if not tarjeta:
        ...
        if es_msi and plazo > 1:
            ...
            if fecha_dt.day <= dia_corte:
                mes_base = fecha_dt.replace(day=1)
            else:
                mes_base = fecha_dt.replace(day=1) + relativedelta(months=1)

            for num in range(1, plazo + 1):
                mes_objetivo = mes_base + relativedelta(months=num - 1)
                # Protegemos contra meses cortos (ej. Febrero 28)
                _, ultimo_dia = monthrange(mes_objetivo.year, mes_objetivo.month)
                dia_real = min(dia_corte, ultimo_dia)

                # Asignamos la fecha exacta del corte como mes_cobro
                mes_cobro = date(mes_objetivo.year, mes_objetivo.month, dia_real)
